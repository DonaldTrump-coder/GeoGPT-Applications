...
class Agent_Processor:
    ...
    def init_prompts(self):
        ...
        #定义模型能够输出的指令
        self.control_prompts="The actions must be outputed in the dict format: { action: parameter }. Here are the actions that can be outputed: 1.'turn left': turn around the drone to its left side with the parameter as degree; 2.'turn right': turn around the drone to its right side with the parameter as degree; 3.'move forward': move the drone to its front side with the parameter as meter; 4.'move backward': move the drone to its back side with the parameter as meter; 5.'move up': move the drone to a higher place with the parameter as meter; 6.'move down': move the drone to a lower place with the parameter as meter; 7.'get image': obtain the next image taken from the drone, which has five camera directions, and the parameter must be one of the ['front', 'down', 'back', 'left', 'right'] (The image is in the form of natural language description, and you must understand it). 8.'take off': control the drone to take off to a certain height with the parameter as meter. 9.'land': when the drone arrives at the right place, land to finish the task, with the parameter as meters per second(landing speed). Usually, 'get image' must be taken after every other action, on order to control the drone accurately. If you can not see the task object or decide what to do, you should also get another image again. Only output an action dict at a time, for example { 'get image': 'front' }."
